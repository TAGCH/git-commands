...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - The staging area is an intermediate area where changes are prepared before they are committed to the Git repository.
   * Working copy - The working copy is the local directory on your machine where you make modifications to files in your Git repository.
   * master - The "master" branch is the default branch that is created when initializing a new Git repository.
   * HEAD - HEAD is a symbolic reference to the most recent commit in the currently checked-out branch.
...
3. There are 2 ways to create a local Git repository.  Briefly descibe each one:
   1. Initializing a Repository in an Existing Directory
      - Using the 'git init' command to create the existing directory into a Git repository.  
   2. Cloning a Remote Repository
      - Using the 'git clone <remote_repository_url>' command to create a local copy of an existing remote Git repository.
...
6. In any project, there are some files and directories that you **should not** commit to git.    
   ...
   - Dependency Management Files: Ex.'requirements.txt', 'Pipfile', 'Pipfile.lock'
   - Compiled Files and Bytecode: Ex. '*.pyc', '*.pyo'
   - Log Files: Ex. '*.log'
   - Build Output and Temporary Files: Ex. '__pycache__/', 'build/', 'dist/'
   - Virtual Environment Directories: Ex. 'venv/', 'env/'
...
## Branch and Merge
```
   git checkout -b test/test_branch
   git merge test/main
...
```
   ...
   - git checkout main
   - git commit -m "C1"
   - git rebase -i overHere --solution-ordering C1
```


## Resources

* [Git Tutorial](https://www.w3schools.com/git/) Learning by Examples.
* [Git commands](https://www.atlassian.com/git/glossary) Basic Git Commands.
